<?php
  ...
  include "ConnectAdminTable.php";
    $sql = "SELECT id, resname,firstname, lastname, address, phone, addmap FROM resuser where id=".$id." ";
...
<div class="row">
  ...
  <div class="col-sm-8" >
    <form action="homemenu.php?page=restaurantUser&menu=allowUser&id=<?php echo $id;?>" method="POST" style="margin-top:10px">
      <h2>แก้ไขผู้ใช้สำหรับส่งอาหาร</h2>
      <div class="form-group" style="margin-top:20px">
        <label for="email">ชื่อร้านอาหาร:</label>
        <input type="text" class="form-control" id="email" placeholder="Enter restaurant name" name="resname" value="<?php echo $data["resname"];?>" >
      </div>
      <div class="form-group" style="margin-top:20px">
      ...
      <div class="form-group" style="margin-bottom:20px">
        <label for="pwd">ที่อยู่แผ่นที่:</label>
        <input type="text" class="form-control" id="pass" placeholder="Enter map address" name="addmap" value="<?php echo $data["addmap"];?>">
      </div>
      <div class="form-group" style="margin-bottom:20px">
